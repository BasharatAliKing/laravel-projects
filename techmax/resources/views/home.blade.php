...
@endsection
@section('content')
  <!--Main Home starts from here-->
  <section id="home" class="bg-home-bg bg-no-repeat bg-center bg-cover 2xl:h-screen ">
    <div class="particles-js  dots" id="particles-js">
      <canvas class="particles-js-canvas-el" width="1332" height="756" style="width: 100%; height: 100%;"></canvas>
    </div>
    <div class="king 2xl:h-screen ">
      <div class="container pt-32 pb-20 flex flex-col md:flex-row 2xl:items-center">
        <div class="md:w-1/2 flex flex-col gap-5">
          <div class="flex gap-6 text-lg">
            <i class="fa-brands fa-facebook-f hover:text-green hover:duration-700 hover:-mt-1"></i>
            <i class="fa-brands fa-youtube hover:text-green hover:duration-700 hover:-mt-1"></i>
            <i class="fa-brands fa-instagram hover:text-green hover:duration-700 hover:-mt-1"></i>
            <i class="fa-brands fa-twitter hover:text-green hover:duration-700 hover:-mt-1"></i>
          </div>
          <div class="flex flex-col gap-2">
            <h2 class="uppercase flex items-center gap-2 text-green font-medium"> <span
                class="h-[7px] w-[7px] rounded-full bg-green"></span> IT Solutions <span
                class="h-[7px] w-[7px] rounded-full bg-green"></span></h2>
            <h1 class="text-3xl sm:text-4xl md:text-4xl lg:text-6xl leading-tight font-bold">Providing The Best
              Services & IT</h1>
            <span class="uppercase text-green text-3xl font-medium">solutions
              <svg xmlns="http://www.w3.org/2000/svg" class="wavey-underline md:-mt-3" width="170" height="auto"
                ...
                <path id="Path_208" data-name="Path 208"
                  ...
                  transform="translate(-504.796 -1565.664)" fill="#00b243" stroke="#00b243" stroke-width="5"></path>
              </svg>
            </span>
            <p class="text-base md:text-lg mt-1">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sed
              architecto dolorum inventore totam adipisci</p>

          </div>
          <div class="flex gap-4">
            <button
              class="px-8 md:px-11 py-1 md:py-3 border-2 rounded-tr-2xl rounded-bl-2xl font-medium text-sm md:text-base border-green hover:duration-700 hover:bg-green hover:text-white">Start
              Now</button>
            <i
              class="fa-solid fa-play w-10 h-10 md:w-14 md:h-14 text-center flex items-center justify-center animate-pulse hover:duration-700 hover:bg-white hover:text-green rounded-full bg-green text-lg"></i>
          </div>
        </div>
        <div class="md:w-1/2 md:-mt-10"><img src="/assets/images/illustration-1.png" data-tilt alt=""></div>
      </div>
    </div>
  </section>
  @include('components.services')
  @include('components.about')
  @include('components.chooseus')
  <!--Clinents satify-->
  <section id="clients" class="bg-timebg bg-no-repeat bg-center bg-cover">
    <div id="counter-section" class="container py-16 grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-4 gap-6">
      <div data-tilt data-aos="fade-up"
        class="border  border-green border-t-[8px] w-[70%] md:w-full mx-auto md:mx-0 p-5 md:p-10 border-t-green bg-[#00b2446c] rounded-md flex flex-col gap-1 items-center">
        <img src="/assets/images/bulb.svg" alt="">
        <h1
          class="flex tabular-nums  text-5xl font-bold mb-2 transition-[_--num] duration-[3s] ease-out [counter-set:_num_var(--num)] supports-[counter-set]:before:content-[counter(num)]"
          x-data="{ shown: false }" x-intersect="shown = true" :class="shown && '[--num:750]'">
          <span class="supports-[counter-set]:sr-only">750</span>+
        </h1>
        <h2 class="text-base md:text-xl font-medium text-center">Finished Projects</h2>
      </div>
      <div data-tilt data-aos="fade-up"
        class="border  border-green border-t-[8px] w-[70%] md:w-full mx-auto md:mx-0 p-5 md:p-10 border-t-green bg-[#00b2446c] rounded-md flex flex-col gap-1 items-center">
        ...
        <h1
          class="flex tabular-nums  text-5xl font-bold mb-2 transition-[_--num] duration-[3s] ease-out [counter-set:_num_var(--num)] supports-[counter-set]:before:content-[counter(num)]"
          x-data="{ shown: false }" x-intersect="shown = true" :class="shown && '[--num:23]'">
          <span class="supports-[counter-set]:sr-only">23</span>+
        </h1>
        <h2 class="text-base md:text-xl font-medium text-center">Created Jobs</h2>
      </div>
      <div data-tilt data-aos="fade-up"
        class="border  border-green border-t-[8px]  w-[70%] md:w-full mx-auto md:mx-0 p-5 md:p-10 border-t-green bg-[#00b2446c] rounded-md flex flex-col gap-1 items-center">
         <i class="fa fa-users text-[44px]"></i>
        <h1
          class="flex tabular-nums  text-5xl font-bold mb-2 transition-[_--num] duration-[3s] ease-out [counter-set:_num_var(--num)] supports-[counter-set]:before:content-[counter(num)]"
          x-data="{ shown: false }" x-intersect="shown = true" :class="shown && '[--num:740]'">
          <span class="supports-[counter-set]:sr-only">740</span>+
        </h1>
        <h2 class="text-base md:text-xl font-medium text-center">Happy Customers</h2>
      </div>
      <div data-tilt data-aos="fade-up"
        class="border  border-green border-t-[8px]  w-[70%] md:w-full mx-auto md:mx-0 p-5 md:p-9 border-t-green bg-[#00b2446c] rounded-md flex flex-col gap-1 items-center">
        <img src="/assets/images/arrow.svg" alt="">
        <h1
          class="flex tabular-nums  text-5xl font-bold mb-2 transition-[_--num] duration-[3s] ease-out [counter-set:_num_var(--num)] supports-[counter-set]:before:content-[counter(num)]"
          x-data="{ shown: false }" x-intersect="shown = true" :class="shown && '[--num:4]'">
          <span class="supports-[counter-set]:sr-only">4</span>+
        </h1>
        <h2 class="text-base md:text-xl font-medium text-center">Years of Experience</h2>
      </div>
    </div>
  </section>
